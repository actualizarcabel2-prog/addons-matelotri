...
from tmdb import (popular_movies, now_playing, top_rated_movies,
                  ...
                  search_multi, movie_details, tv_details, tv_season,
                  genres_movie, genres_tv, discover_movie, discover_tv,
                  poster, backdrop)
from trial import can_play, get_max_quality, get_status_text
...
def add_dir(label, action, params=None, icon_name="", plot="",
    ...
    art["fanart"] = fanart_img or os.path.join(ADDON_PATH, "fanart.png")
    li.setArt(art)
    if plot:
...
def add_movie_item(movie):
    title = movie.get("title", "Sin titulo")
    ...
    url = build_url({"action": "play_movie", "movie_id": movie_id,
...
def add_tv_item(show):
    title = show.get("name", "Sin titulo")
    year = show.get("first_air_date", "")[:4]
    label = "{} ({})".format(title, year) if year else title
    tv_id = show.get("id", 0)
    poster_url = poster(show.get("poster_path"))
    fanart_url = backdrop(show.get("backdrop_path"))
    url = build_url({"action": "tv_seasons", "tv_id": tv_id, "title": title})
    li = xbmcgui.ListItem(label)
    li.setInfo("video", {"title": title, "year": int(year) if year else 0,
                          "plot": show.get("overview", ""), "mediatype": "tvshow"})
    li.setArt({"poster": poster_url, "thumb": poster_url,
               "fanart": fanart_url or os.path.join(ADDON_PATH, "fanart.png"),
               "icon": poster_url})
...
def main_menu():
    ...
    add_dir("[COLOR gold]Peliculas[/COLOR]", "peliculas", icon_name="peliculas.png")
    ...
    add_dir("[COLOR gold]Musica[/COLOR]", "musica", icon_name="musica.png")
    ...
    add_dir("[COLOR gold]Ajustes[/COLOR]", "ajustes", icon_name="ajustes.png", is_folder=False)
    xbmcplugin.endOfDirectory(HANDLE)


def menu_peliculas():
    add_dir("[COLOR gold]Populares[/COLOR]", "movie_list", {"cat": "popular"}, "peliculas.png")
    add_dir("[COLOR gold]Estrenos[/COLOR]", "movie_list", {"cat": "now_playing"}, "peliculas.png")
    add_dir("[COLOR gold]Mejor valoradas[/COLOR]", "movie_list", {"cat": "top_rated"}, "peliculas.png")
    add_dir("[COLOR gold]Por genero[/COLOR]", "movie_genres", icon_name="peliculas.png")
    add_dir("[COLOR gold]Por ano[/COLOR]", "movie_years", icon_name="peliculas.png")
    xbmcplugin.endOfDirectory(HANDLE)


def menu_series():
    add_dir("[COLOR gold]Populares[/COLOR]", "tv_list", {"cat": "popular"}, "series.png")
    add_dir("[COLOR gold]En emision[/COLOR]", "tv_list", {"cat": "on_the_air"}, "series.png")
    add_dir("[COLOR gold]Mejor valoradas[/COLOR]", "tv_list", {"cat": "top_rated"}, "series.png")
    add_dir("[COLOR gold]Por genero[/COLOR]", "tv_genres", icon_name="series.png")
    xbmcplugin.endOfDirectory(HANDLE)


def menu_dibujos():
    add_dir("[COLOR gold]Peliculas animadas[/COLOR]", "movie_list",
...
def menu_anime():
    ...
    add_dir("[COLOR gold]Anime - Peliculas[/COLOR]", "movie_list",
            {"cat": "discover", "genre": "16"}, "anime.png")
    xbmcplugin.endOfDirectory(HANDLE)


def menu_documentales():
    add_dir("[COLOR gold]Documentales - Peliculas[/COLOR]", "movie_list",
...
def menu_musica():
    add_dir("[COLOR gold]Peliculas musicales[/COLOR]", "movie_list",
...
def list_movies(params):
    ...
    if page < total:
        add_dir("[COLOR gold]Siguiente ({}/{})[/COLOR]".format(page+1, total),
                "movie_list", {"cat": cat, "page": page+1, "genre": genre, "year": year})
...
def list_tv(params):
    ...
    if page < total:
        add_dir("[COLOR gold]Siguiente ({}/{})[/COLOR]".format(page+1, total),
                "tv_list", {"cat": cat, "page": page+1, "genre": genre})
    xbmcplugin.setContent(HANDLE, "tvshows")
    xbmcplugin.endOfDirectory(HANDLE)


def list_movie_genres():
    for g in genres_movie().get("genres", []):
        add_dir("[COLOR gold]{}[/COLOR]".format(g["name"]),
                "movie_list", {"cat": "discover", "genre": g["id"]}, "peliculas.png")
    xbmcplugin.endOfDirectory(HANDLE)


def list_tv_genres():
    for g in genres_tv().get("genres", []):
        add_dir("[COLOR gold]{}[/COLOR]".format(g["name"]),
                "tv_list", {"cat": "discover", "genre": g["id"]}, "series.png")
    xbmcplugin.endOfDirectory(HANDLE)


def list_movie_years():
    import datetime
    for y in range(datetime.datetime.now().year, 1990, -1):
        add_dir("[COLOR gold]{}[/COLOR]".format(y),
                "movie_list", {"cat": "discover", "year": y}, "peliculas.png")
    xbmcplugin.endOfDirectory(HANDLE)
...
def list_seasons(params):
    ...
    for s in data.get("seasons", []):
        ...
        url = build_url({"action": "tv_episodes", "tv_id": tv_id,
                          "season": snum, "title": title})
        li = xbmcgui.ListItem("[COLOR gold]Temporada {}[/COLOR] ({} eps)".format(snum, ep_count))
        li.setArt({"poster": poster_url, "thumb": poster_url,
...
def list_episodes(params):
    ...
    for ep in data.get("episodes", []):
        ...
        url = build_url({"action": "play_episode", "tv_id": tv_id,
                          "season": season, "episode": epnum, "title": title})
        li = xbmcgui.ListItem("[COLOR gold]{}[/COLOR]".format(label))
        still = ep.get("still_path")
        li.setArt({"thumb": poster(still) if still else "",
                   "fanart": os.path.join(ADDON_PATH, "fanart.png")})
        li.setInfo("video", {"plot": ep.get("overview", ""),
                              "mediatype": "episode",
...
def play_movie(params):
    if not can_play():
        xbmcgui.Dialog().ok("[COLOR gold]Matelotri Cinema[/COLOR]",
            "[COLOR red]Tu prueba de 3 dias ha expirado.[/COLOR]")
        return
    ...
    pDialog.update(80)
    links = filter_by_quality(links, get_max_quality())
    pDialog.close()

    if not links:
        xbmcgui.Dialog().ok("[COLOR gold]Matelotri Cinema[/COLOR]",
            "No se encontraron enlaces para:\n{}".format(title))
        return
    ...
    if len(links) == 1:
        ...
        idx = xbmcgui.Dialog().select("[COLOR gold]Seleccionar enlace[/COLOR]", names)
        if idx >= 0:
...
def play_episode(params):
    ...
    pDialog = xbmcgui.DialogProgress()
    pDialog.create("[COLOR gold]Matelotri Cinema[/COLOR]",
                   "Buscando: {} S{:02d}E{:02d}...".format(title, int(season), int(episode)))
    pDialog.update(10)
...
def router(paramstring):
    ...
    if not action:
        ...
        xbmcgui.Dialog().ok("[COLOR gold]Matelotri Cinema[/COLOR]", "Proximamente...")
    elif action == "ajustes":
        ADDON.openSettings()
